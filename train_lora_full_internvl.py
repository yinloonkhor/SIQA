...
VISION_LORA_SUFFIXES = ("qkv", "proj", "projection_layer")
...
def _is_vision_module(module_name: str) -> bool:
    ...
    return any(marker in module_name for marker in vision_markers)
...
def configure_image_processor(processor: Any, min_pixels: int, max_pixels: int) -> None:
    ...
    if image_processor is None:
        raise ValueError("Loaded processor does not expose image_processor for pixel-limit configuration.")
    ...
    # Keep the pixel cap separate from model-native resize settings. InternVL processors use
    # square `size` plus patch limits, so overwriting `image_processor.size` with pixel counts
    # breaks preprocessing.
    setattr(processor, "_siqa_image_min_pixels", int(min_pixels))
    setattr(processor, "_siqa_image_max_pixels", int(max_pixels))
...
def resolve_processor_max_image_pixels(processor: Any) -> int:
    max_pixels = getattr(processor, "_siqa_image_max_pixels", None)
    if max_pixels is None:
        raise ValueError(
            "Processor max-image pixel cap is not configured. "
            "Call configure_image_processor() before creating collators."
        )
...
class MixedTrainCollator:
    def __init__(self, processor: Any, max_length: int, score_tokens: ScoreTokenHelper) -> None:
        ...
        self.max_image_pixels = resolve_processor_max_image_pixels(processor)
    ...
    def __call__(self, batch: list[MixedSiqaExample]) -> dict[str, Any]:
        ...
        full_token_id_lists: list[list[int]] = []
        ...
        for example in batch:
            ...
            prompt_token_id_lists.append(self._tokenize_chat_text(prompt_text))
            full_token_id_lists.append(self._tokenize_chat_text(full_text))
        ...
        for batch_index, example in enumerate(batch):
            full_token_ids = full_token_id_lists[batch_index]
            prompt_token_ids = prompt_token_id_lists[batch_index]
            sequence_end = len(full_token_ids)
            prompt_len = self._resolve_prompt_prefix_length(
            ...
            if example.family == SIQA_S_FAMILY:
                answer_ids = full_token_ids[prompt_len:]
                quality_offset = self._find_quality_position(answer_ids)
                if quality_offset is None:
                    decoded_answer = self.processor.tokenizer.decode(answer_ids, skip_special_tokens=False)
                    raise ValueError(
                        f"Could not locate quality token for task={example.score_task} "
                        f"target={example.target_text!r} decoded_answer={decoded_answer!r}"
                    )
    ...
    def _tokenize_chat_text(self, text: str) -> list[int]:
    ...
    def _resolve_prompt_prefix_length(
        self,
        full_token_ids: list[int],
        prompt_token_ids: list[int],
    ) -> int:
        prefix_length = 0
        max_prefix_length = min(len(full_token_ids), len(prompt_token_ids))
        while prefix_length < max_prefix_length:
            if full_token_ids[prefix_length] != prompt_token_ids[prefix_length]:
                break
            prefix_length += 1
...
def main() -> None:
    model_name = "OpenGVLab/InternVL3_5-2B-HF"
    ...
    root = "TrainSet"
    output_dir = Path("checkpoints/internvl3_5-2b-hf-siqa-mixed-lora_frac_05")
    ...
    siqa_u_epoch_multiplier = 0.5 # Full sample: 104021
